...
1. **Wallpaper layer**: the package ships `src/styles/wallpaper.css`, imported by the shell's base.css after every ui-theme token sheet. The sheet reads four document-scoped custom properties (`--dshw-image/size/repeat/dim`) and paints the image plus the dim scrim as the `html` canvas background, so it sits behind every surface and can never cover the app. The browser half's `WallpaperPresenter` writes those properties from the durable section and retracts exactly its own writes on dispose, mirroring ui-layout's `ThemePresenter` discipline. The body's own opaque base background covers the canvas in `solid` mode, and the translucent token override reveals it in `translucent`/`glass` modes.
...
3. **Durable section**: the `ui-appearance` settings namespace (wallpaper kind/value/fit/dim plus surface) is registered host-side and bound in the browser through `ctx.settingsScope`. The api-proxy `WEB_SETTINGS_NAMESPACES` allowlist admits the namespace — the explicit decision point that makes any Settings registration remotely readable or writable.
...
**Reuse `body`'s own background (html/body background-image).** Rejected: the base.css body background is the theme's base color and sits above the canvas; painting the wallpaper on `html` keeps it behind the body without any stacking tricks. The original `body::before`/`body::after` with `z-index: -1` attempt was abandoned after it covered the app in the real shell — negative-z-index pseudos are fragile against the shell's stacking contexts, so the canvas background is the load-bearing choice.
...
The feature ships as a self-contained package with zero changes to product DOM or the theme registry: stopping the plugin fiber retracts every layer, and the shell's only footprint is one base.css import and the bundle roster rows. Because the wallpaper is the html canvas background, it can never obscure content; it is only visible through translucent surfaces — the intended composition, but a user who picks a wallpaper while panels are `solid` sees nothing until they change the surface mode, so the settings page copy states the composition explicitly.
